  ...
  <div class="m-0 p-2">
    <div class="datatable-toolbar pb-2">
      ...
      <input type='button' class='btn' v-for='button in buttons'
        :key='button.value' :value='button.value' v-on:click="button.event"/>
    </div>
      ...
      <tbody>
        <tr v-for='data in dataList' :key='data.id'>
          <td v-for="column in columns" :key="column.title"
            v-if="column.buttons == null && column.convert == null">{{ data[column.field] }}</td>
          <td v-else-if="column.convert != null">{{ column.convert(data[column.field]) }}</td>
          <td v-else>
            <input type='button' class="datatable-button mr-2"
              v-for="button in column.buttons" :key="button.value"
              v-if="button.condition ? button.condition(data) : true"
              v-bind:value="button.convert != null ? button.convert(data) : button.value"
              v-on:click="button.event(data, dataList)"/>
          </td>
        </tr>
      </tbody>
    </table>
